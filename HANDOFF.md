...
- **Phases 0–9 COMPLETE**: core editor/collab stack, auth + orgs + permissions, sharing, folders + sidebar, daemon local sync, comments/suggestions/discussions, version history, git push/pull, Docker + CI, onboarding path split, agent platform + MCP server, and Google Docs-style editing modes.
- Latest large milestone landed in commit `4a9eb28` ("Phase 9 + 9b + editing modes").
...
- `apps/web/middleware.ts` — edge cookie check, protects /doc/_, /org/_, /trash
...
Current focus is production hardening: keep lint/typecheck/test/build green, complete the in-progress anchored comment panel polish, and run end-to-end validation (web app, sync server, CLI daemon, and Docker paths).
